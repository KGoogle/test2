...
def generate_html(science_data, nasa_data):
    json_data = json.dumps(science_data, ensure_ascii=False)
    
    if nasa_data:
        nasa_html = f"""
            <img src="{nasa_data.get('hdurl') or nasa_data.get('url')}" class="nasa-img">
            <div class="nasa-info">
                <span class="nasa-tag">NASA APOD TODAY</span>
                <div class="nasa-title">{nasa_data.get('title', 'No Title')}</div>
                <p class="nasa-desc">{nasa_data.get('explanation', '')}</p>
                <div style="font-size: 0.8rem; color: #555; margin-top:15px;">
                    Copyright: {nasa_data.get('copyright', 'NASA')} | Date: {nasa_data.get('date', '')}
                </div>
            </div>
        """
    else:
        nasa_html = '<div style="padding: 50px; text-align: center; color: #444;">NASA 데이터를 가져올 수 없습니다.</div>'
...
<head>
    ...
    <style>
        ...
        header {{ 
            position: relative; text-align: center; padding: 80px 20px 60px 20px;
            overflow: hidden; background: #000;
        }}
        #universe {{ position: absolute; top: 0; left: 0; width: 100%; height: 100%; z-index: 0; }}
        .header-content {{ position: relative; z-index: 1; }}
        header h1 {{
            margin-top: 30px; font-size: 18px; color: #ffffff;
        ...
        .container {{ max-width: 1200px; margin: 0 auto; padding: 40px 20px; min-height: 100vh; }}
        .nasa-hero {{ margin-bottom: 50px; border-radius: 15px; overflow: hidden; background: #0a0a0a; border: 1px solid #222; }}
        .nasa-img {{ width: 100%; height: auto; max-height: 550px; object-fit: cover; display: block; }}
        ...
        .data-box {{ background: var(--card-bg); border: 1px dashed #444; padding: 80px 20px; border-radius: 15px; text-align: center; grid-column: 1 / -1; }}
...
<body>
    ...
    <div class="container">
        <div id="nasa-today" class="nasa-hero">
            {nasa_html}
        </div>
    ...
    <script>
        ...
        function render() {{
            const content = fullData[currentField];
            const container = document.getElementById('main-content');
            let html = '<div class="card-grid">';
            if (currentType === 'news') {{
                html += content.news.map(n => `<a href="${{n.link}}" class="card"><div class="card-title">${{n.title}}</div><div class="card-meta">${{n.date}}</div></a>`).join('');
            }} else if (currentType === 'papers') {{
                html += content.papers.map(p => `<a href="${{p.url}}" class="card"><div class="card-title">${{p.title}}</div><div class="card-abstract">${{p.abstract}}</div><div class="card-meta">${{p.authors}}</div></a>`).join('');
            }} else if (currentType === 'comm') {{
                html += content.videos.map(v => `<a href="${{v.link}}" class="card"><img src="${{v.thumbnail}}" class="video-thumb"><div class="card-title">${{v.title}}</div></a>`).join('');
            }} else if (currentType === 'data') {{
                html = `<div class="data-box"><div style="font-size:1.8rem; font-weight:bold; color:var(--accent);">${{content.data}}</div></div>`;
            }} else if (currentType === 'events') {{
                html = '<div>' + content.events.map(e => `<div class="event-item"><div style="color:var(--text-sub);">${{e.date}}</div><div>${{e.title}}</div></div>`).join('') + '</div>';
            }}
            if (currentType !== 'data' && currentType !== 'events') html += '</div>';
