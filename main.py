...
<body>
    ...
    <script type="module">
        ...
        async function initBrain() {{
            ...
            brainRenderer = new THREE.WebGLRenderer({{ antialias: false }});
            brainRenderer.setSize(width, height);
            brainRenderer.setPixelRatio(Math.min(window.devicePixelRatio, 1));
    ...
    canvas.width = 64; // 크기를 줄여 메모리 절약 (128 -> 64)
    canvas.height = 64;
    const context = canvas.getContext('2d');
    
    // 중심에서 바깥으로 퍼지는 그라데이션 (빛나는 효과 흉내)
    const gradient = context.createRadialGradient(32, 32, 0, 32, 32, 32);
    
    // 색상을 더 밝고(Hot) 투명하게 조절
    gradient.addColorStop(0, 'rgba(255, 255, 255, 1)');     // 중심: 완전 흰색
    gradient.addColorStop(0.3, 'rgba(180, 200, 255, 0.5)'); // 중간: 푸른 빛
    gradient.addColorStop(0.5, 'rgba(100, 150, 255, 0.2)'); // 외곽: 흐릿
    gradient.addColorStop(1, 'rgba(0, 0, 0, 0)');           // 끝: 투명
    ...
    entries.forEach(entry => {{
        if (entry.isIntersecting) {{
            if (currentField === "천문·우주") animateUniverse();
            else if (currentField === "인지·신경" && brainInitialized) animateBrain();
        }} else {{
            if (animationIdUniverse) cancelAnimationFrame(animationIdUniverse);
            if (animationIdBrain) cancelAnimationFrame(animationIdBrain);
        }}
